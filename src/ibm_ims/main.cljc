(ns ibm_ims.main
  "Kotodama WASM entrypoint for the Ibm_ims clean-room actor (L4) — Clojure port.
  ...
  facts (`ibm_ims.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ibm_ims")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ibm_ims.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ibm_ims-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ibm_ims-compat/ready)
